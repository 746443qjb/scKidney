...
#'
#' @param seurat A Seurat object that has been normalized, scaled, and processed with PCA.
...
#' @export
KidneyPC <- function(seurat, cum = 90, var = 5) {
  ...
  p <- ggplot(plot_df, aes(x = cumu, y = pct, label = rank, color = rank > pcs)) +
    ...
    geom_hline(yintercept = min(pct[pct > var]), color = "grey", linetype = "dashed") +
